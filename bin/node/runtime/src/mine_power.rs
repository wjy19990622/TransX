...
use support::{debug, ensure, decl_module, decl_storage, decl_event, dispatch::Result, Parameter,
...
/// `ChainRunDays`通过let chain_run_days = block_number / BlockNumber::from(DAYS) + BlockNumber::from(1u32);计算而来。
...
pub struct PowerInfoStore<Storage, BlockNumber>(rstd::marker::PhantomData<(Storage, BlockNumber)>);
...
pub struct TokenPowerInfoStore<Storage, BlockNumber>(rstd::marker::PhantomData<(Storage, BlockNumber)>);
...
pub struct MinerPowerInfoStore<Storage, AccountId, BlockNumber>(rstd::marker::PhantomData<(Storage, AccountId, BlockNumber)>);
